...
class AudioCutter:
    ...
    def _check_start_and_end_time(self, start_time, end_time):
        ...
        if not isinstance(start_time, (int, float)):
            raise Exception("start_time must be of type int or flaot. Type {} was given".format(type(start_time)))
        if not isinstance(end_time, (int, float)):
            raise Exception("end_time must be of type int or flaot. Type {} was given".format(type(start_time)))
        ...
        if start_time == START_TIME_DEFAULT:
            ...
            if ( start_time < 0):
                raise Exception("start_time must be positive or zero. {} was given.".format(start_time))
            elif (start_time > audio_duration):
                raise Exception("start_time must be lower than audio duration."
                                "start_time of {} and audio_duration {} were given.".format(start_time, audio_duration))
            elif (start_time >= end_time):
                raise Exception("start_time must be lower than end_time."
                                "start_time of {} and end_time {} were given.".format(start_time, end_time))
        ...
        if end_time == END_TIME_DEFAULT:
            ...
            if (end_time < 0):
                raise Exception("end_time must be positive or zero. {} was given.".format(start_time))
    ...
    def cut_audio(self, start_time, end_time):
        ...
        return AudioSignal( self.audioSignal.get_data()[start_time_frame_index:end_time_frame_index],
                            self.audioSignal.get_sample_rate())

    def slice_signal_in_n_equal_segments(self, n, start_time=START_TIME_DEFAULT, end_time=END_TIME_DEFAULT):
    ...
    def slice_signal_in_multi(self, output_segment_duration, start_time=START_TIME_DEFAULT, end_time=END_TIME_DEFAULT):
        """
        ...
        nr_segments = int( float((end_time - start_time)) / self._output_segment_duration )
